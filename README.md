...
- **See / edit:** tap a session that has laps to see them read-only: the
  ...
  lap time summary and a table of every lap in the order they're entered (lap, start,
  ...
  *All time best* (across every event at the track with the same
  ...
  columns line up from session to session; on a phone, a lap's note goes
  on its own line under it. The tab shows how many
