...
export function EventPicker({ events, active, onChange, onOpenDetails }: Props) {
  ...
  return (
    <div className="relative min-w-0 pl-1">
      ...
      >
        <TrackIcon trackId={active.trackId} size={26} className="shrink-0 text-gray-700" title={`${active.name} track`} />
        ...
        <>
          ...
          <div className="absolute left-0 top-full z-20 mt-2 min-w-[240px] rounded-xl border border-gray-200 bg-white p-1 shadow-xl">
            {events.map(e => (
              ...
              >
                <TrackIcon trackId={e.trackId} size={24} className="shrink-0 text-gray-700" title={`${e.name} track`} />
                ...
                {e.id === active.id && <Check size={14} className="text-blue-500 ml-1 shrink-0" />}
              </button>
            ))}
          </div>
        </>
